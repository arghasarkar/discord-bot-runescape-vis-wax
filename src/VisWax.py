...
class VisWax:
    ...
    def __fetch_webpage__(self):
        ...
        #     "Sec-Fetch-User":"?1",
        #     "Upgrade-Insecure-Requests":"1",
        #     "Cookie": "global__user-visited=true;performedRefererCheck=true;",
        #     "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.104 Safari/537.36"
        #
        # }

        print("A002: Fetching webpage")
        print("A023: Fetching with socks proxy_server")
        ip_addr = "116.203.147.190"
        port = "2806"
        proxy_server = f"socks5h://{ip_addr}:{port}"
        print(f"proxy_server: {proxy_server}")

        # resp = requests.get(self.URL, headers=headers)
        resp = requests.get(self.URL,
                            proxies=dict(http=proxy_server, https=proxy_server),
                            headers=headers)
